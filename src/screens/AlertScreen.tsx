        ...
        {notifPermission !== "granted" && (
          <div className="mx-4 mt-4 p-4 bg-white border border-slate-100 rounded-2xl flex items-center justify-between gap-3 shadow-sm">
            <div className="flex items-start gap-3 min-w-0">
              <div className="w-9 h-9 rounded-lg border border-slate-200 flex items-center justify-center shrink-0">
            ...
            <button
              ...
              className="shrink-0 text-[13px] font-semibold text-white bg-blue-600 active:bg-blue-700 px-3.5 py-2 rounded-lg transition-colors"
            >
              허용하기
            </button>
          </div>
      ...
      <section className="px-4 pt-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[13px] text-white font-semibold bg-blue-600 active:bg-blue-700 px-3.5 py-2 rounded-lg transition-colors"
          >
          ...
          <div className="divide-y divide-slate-100 border-t border-slate-200">
            {records.map((r) => (
              <div key={r.id} className="flex items-start gap-3 py-3.5">
                {/* 알림 종류를 파스텔 배경으로 구분하지 않는다. 읽음 여부는
                    글자 굵기와 점 하나로 충분히 드러난다. */}
                <Bell className="w-4 h-4 text-slate-400 shrink-0 mt-0.5" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p
                      className={`text-[15px] tracking-tight ${
                        r.read ? "font-medium text-slate-500" : "font-semibold text-slate-900"
                      }`}
                    >
                      {r.title}
                    </p>
                    {!r.read && <span className="w-[6px] h-[6px] rounded-full bg-blue-600 shrink-0" />}
                  </div>
                  <p className="text-[13px] text-slate-500 mt-1 leading-relaxed">{r.body}</p>
                  <p className="text-[12px] text-slate-400 mt-1.5">{r.time}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                alert.active ? "border border-slate-300 text-slate-900" : "border border-slate-200 text-slate-300"
              }`}
            >
              {alert.active ? "활성" : "꺼짐"}
            </span>
