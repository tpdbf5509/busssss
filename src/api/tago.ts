...
async function callTagoApi(path: string, params: Record<string, string>): Promise<RawTagoField[]> {
  ...
  let res: Response;
  try {
    res = await fetch(url, { headers: SUPABASE_ANON_KEY ? { Authorization: `Bearer ${SUPABASE_ANON_KEY}`, apikey: SUPABASE_ANON_KEY } : undefined });
  } catch {
...
export async function getRouteNoList(routeNo: string, cityCode: string = JEONJU_CITY_CODE) {
  return callTagoApi("/BusRouteInfoInqireService/getRouteNoList", { cityCode, routeNo, numOfRows: "20", pageNo: "1" });
}
export async function getRouteAcctoBusLcList(routeId: string, cityCode: string = JEONJU_CITY_CODE) {
  return callTagoApi("/BusLcInfoInqireService/getRouteAcctoBusLcList", { cityCode, routeId, numOfRows: "50", pageNo: "1" });
}
export async function getSttnNoList(nodeNm: string, cityCode: string = JEONJU_CITY_CODE) {
  if (!nodeNm.trim()) return [];
  return callTagoApi("/BusSttnInfoInqireService/getSttnNoList", { cityCode, nodeNm: nodeNm.trim(), numOfRows: "30", pageNo: "1" });
}
